    ...
    <!-- END Navigation -->
	  ...
	  <div class="row">
        ...
        <div class="span12">
          ...
          <h1>HAN <small>Compose a Newsletter</small></h1>  
      ...
      <div class="row">
      <form id="sendmail" target="load_frame"  name="sendmail" method="post" action="<?php echo site_url('/');?>han/send_email" >
       <div class="span4">

       		<div id="div_recipients">
            <?php $this->admin_model->show_email_recipients('han');?>	
       	 	</div>
        </div>
        ...
        <div class="span8">
          ...
          <div class="loading_img" id="loading_img">
              ...
              <h3>HAN <small>Compose a Newsletter</small></h3>  
                    ...
                    <div class="btn-group">
                      ...
                      <ul class="dropdown-menu">
                        <li><a tabindex="-3" onClick="select_rec('han');" href="javascript:void(0);">HAN Members</a></li>
                      </ul>
                    </div>
...
$(document).ready(
	function()
	{
		$('#redactor_content').redactor({ 
				fileUpload: '<?php echo site_url('/')?>my_images/redactor_add_document',	
				imageUpload: '<?php echo site_url('/')?>my_images/redactor_add_image',
				buttons: ['html', '|', 'formatting', '|', 'bold', 'italic', 'deleted', '|', 
				'unorderedlist', 'orderedlist', 'outdent', 'indent', '|',
				'video','file','image', 'table', 'link','|',
...
function select_rec(str){
		...
		$.ajax({
			...
			cache: false,
			data:{mailbody: str},
			url: '<?php echo site_url('/').'han/show_email_recipients/';?>'+str ,
...
function preview(){
		...
		$.ajax({
			...
			cache: false,
			data:{mailbody: str},
			url: '<?php echo site_url('/').'han/preview_news/';?>' ,
			success: function (data) {
				$('#preview_button').show();	
				preview.html(data);
				loading.removeClass('loading_img');
		...
		if(!$('#title').val().length == 0){
				...
				$('#modal-email').bind('show', function() {
						...
						$('#send_email_yes').unbind('click').click( function() { 	
								...
								var frm = $('#sendmail');
								barcover.show();
								frm.attr('action','<?php echo site_url('/').'han/send_email/';?>');
								...
								$('#send_email_yes').html('<img src="<?php echo base_url('/').'img/load.gif';?>" /> Sending...');
									...
									$.ajax({
										...
										cache: false,
										data: frm.serialize(),
										url: '<?php echo site_url('/').'han/send_email/';?>' ,
										success: function (data) {
											//barcover.hide();
