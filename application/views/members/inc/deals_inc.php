            ...
            <form id="deal-add" name="deal-add" method="post" action="<?php echo site_url('/');?>deals/add_deal" class="form-horizontal">
                             <fieldset>
                                         ...
                                         <div class="control-group <?php if(isset($special_type) && ($special_type == 'special')){ echo ' hide';}?>" id="quantity_div">
                                          ...
                                          <?php
										  //CYMOT SPECIAL CATEGORIES
										  if($bus_id == 514 ){
										  ?>
                                          <div class="control-group">
                                            <label class="control-label" for="cymot_cat">Select Cymot Category</label>
                                            <div class="controls">
                                                    
                                                        <select class="span12" id="cymot_cat" name="cymot_cat" placeholder="">
                                                          <option>Select Cymot Category</option>
                                                          <option value="colour-perfect" <?php if(isset($cymot_cat) && $cymot_cat == 'colour-perfect'){echo 'selected';}?>>Colour Perfect</option>
                                                           <option value="greensport" <?php if(isset($cymot_cat) && $cymot_cat == 'greensport'){echo 'selected';}?>>Greensport</option>
                                                            <option value="midas" <?php if(isset($cymot_cat) && $cymot_cat == 'midas'){echo 'selected';}?>>Midas</option>
                                                             <option value="namsafe" <?php if(isset($cymot_cat) && $cymot_cat == 'namsafe'){echo 'selected';}?>>Namsafe</option>
                                                              <option value="tooltech" <?php if(isset($cymot_cat) && $cymot_cat == 'tooltech'){echo 'selected';}?>>Tooltech</option>
                                                              
                                                        
                                                        </select>
                                                        
                                                        <span class="help-block" style="font-size:11px">Please select a Cymot specialized category for the deal</span>
                                          ...
                                          <div class="control-group">
                                           ...
                                           <?php 
											if(isset($deal_id) && $is_active == 'N'){
												...
												echo '<a onclick="submit_deal('.$deal_id.')" class="btn btn-inverse pull-right" style="margin-left:10px;display:block" id="btn_submit_deal">
												  <i class="icon-play icon-white"></i> Publish Deal</a>';
													
												
											}else{
												
												echo '<a class="btn btn-inverse pull-right" style="margin-left:10px;display:none" id="btn_submit_deal">
												  <i class="icon-play icon-white"></i> Publish Deal</a>';
...
<div id="modal-deal-delete" class="modal hide fade">
    ...
    <div class="modal-header">
      <a href="#" onclick="javascript:$('#modal-deal-delete').modal('hide')" class="close">&times;</a>
    ...
    <div class="modal-footer">
      ...
      <a href="#" onclick="javascript:$('#modal-deal-delete').modal('hide')" class="btn btn-secondary">No</a>
    </div>
    ...
    <div class="modal-body">
     <p>Please select the images you want to accompany the Deal</p>
 		...
 		<div class="control-group">
           ...
           <span class="help-block"  style="font-size:11px">
             Image dimensions required 780px wide by 540px in height</span>
...
function add_deal_image(){
	...
	$('#modal-deal-img').unbind('show').bind('show', function() {
...
function delete_deal(id){
	...
	$('#modal-deal-delete').appendTo("body").bind('show', function() {
		//var id = $(this).data('id'),
			...
			removeBtn.click(function(e) { 
				...
				e.preventDefault();
						...
						$.ajax({
							type: 'post',
							url: '<?php echo site_url('/').'deals/delete_deal/';?>' ,
							data: {deal_id: id},
							success: function (data) {
								 
								 $('#modal-deal-delete').modal('hide');
								 $('#deal_img').html(data);
								 load_tab(<?php echo $bus_id;?>,'deals');
							}
						});
			});
	}).modal({ backdrop: true });
...
function submit_deal(id){
	...
	$.ajax({
			type: 'get',
			cache: false,
			url: '<?php echo site_url('/').'deals/publish_deal/';?>'+id ,
			success: function (data) {
					cont.removeClass('loading_img'); 
				    cont.html(data);
					setTimeout("load_tab(<?php echo $bus_id;?>,'deals')",2000); 
