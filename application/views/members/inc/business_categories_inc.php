 ...
 //Roland Ihms
 //Get Map Details
$cat_details = $this->members_model->get_main_categories();
$current_cats = $this->members_model->get_current_categories($ID);
...
<div id="sel_cats">
    ...
    foreach($current_cats->result() as $row1){
        ...
        echo '<a href="javascript:void(0)" onclick="delete_cat('.$cat_id_cur.','.$ID.')" style="margin-bottom:5px;" class="btn del_cat" rel="tooltip" title="Remove '.$this->members_model->get_category_name($cat_id_cur).' category" >'.$this->members_model->get_category_name($cat_id_cur).' <i class="icon-remove"></i></a> ';
...
function delete_cat(id, bus_id){
	...
	$('#modal-cat-delete').appendTo("body").bind('show', function() {
		//var id = $(this).data('id'),
