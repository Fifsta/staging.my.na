...
<script type="text/javascript" src="<?php echo S3_URL;?>scratch_card/js/ipad/scratcher.js"></script>
...
<script type="text/javascript">
  ...
  var BACKGROUND_IMAGE = '<?php echo base_url('/').'img/timbthumb.php?src='. S3_URL .'scratch_card/volleyball/prizes/'. $img_file;?>&h='+w_height+'&w='+w_width;
 
 
  var BASE_PATH = '<?php echo S3_URL;?>scratch_card/expo2013';
  var USER_LOG_ID = '';
  ...
  $(document).ready(function() {
	  ...
	  $('#scratcher1').attr('width',w_width);
 	  $('#scratcher1').attr('height',w_height);
    ...
    function askDoneScratching()
    {
		...
		?>
			...
			$('#modal-win').bind('show', function() {
				...
				$('#win_msg').html('<img src="<?php echo S3_URL.'scratch_card/volleyball/prizes/'.$img_prize;?>" class="img-polaroid pull-left" style="width:60px;margin-right:20px;" />Congratulations, you have won a prize. PLease come to the Intouch Field to claim your prize. Claim it now and have !na Day');
				...
				removeBtn.unbind('click').click(function(e) { 
					...
					window.location.href = '<?php echo site_url('/');?>volleyball/';
			...
			$('#modal-win').bind('show', function() {
				...
				if($('#scratchPct').val() > 40){
					...
					removeBtn.unbind('click').click(function(e) { 
						...
						window.location = '<?php echo site_url('/');?>volleyball/';
