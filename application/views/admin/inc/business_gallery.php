...
<?php
 //Get GALLERY
$gal_details = $this->admin_model->get_gallery($ID);
...
<form action="<?php echo site_url('/')?>members/add_gallery_images/" method="post" accept-charset="utf-8" id="add-gal" name="add-gal" enctype="multipart/form-data">  
 ...
 <legend>Add gallery images</legend>
      ...
      <div class="controls">
         <?php if($BUSINESS_LOGO_IMAGE_NAME != ''){
		  $fake_file = S3_URL.'assets/business/photos/'.$BUSINESS_LOGO_IMAGE_NAME;
		   //$fake_file = base_url('/').'img/timbthumb.php?src='.base_url('/').'assets/business/photos/'.$BUSINESS_LOGO_IMAGE_NAME .'&w=100&h=100&q=100';
		   ?>
...
if($gal_details->result()){
	...
	$this->admin_model->show_all_gallery_images($bus_id);
 ...
 */
 ?> 
<div class="modal hide fade" id="modal-img-delete">
...
function delete_gallery_img(id){
	...
	$('#modal-img-delete').bind('show', function() {
		//var id = $(this).data('id'),
