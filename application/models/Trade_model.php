...
class Trade_model extends CI_Model
{
	...
	public function populate_city($city_current)
	{
		...
		if ($query->num_rows() > 0)
		{
			...
			echo '<select name="location" id="location_slct" class="form-control">
				...
				if ($val == true)
				{
					...
					if ($bus_id != 0)
					{
						...
						$query = $this->db->query("SELECT u_client.*, products.*, product_images.img_file, product_extras.extras,
													agent.ID as agent_id, agent.CLIENT_CELLPHONE as agent_cell,agent.CLIENT_EMAIl as agent_email
													,group_concat(DISTINCT(client.CLIENT_EMAIL)) as emails
													FROM u_client
													...
													LEFT JOIN u_client as client ON client.ID = i_client_business.CLIENT_ID
						  							...
						  							LEFT JOIN u_client as agent ON agent.ID = product_extras.property_agent
													LEFT JOIN product_images ON products.product_id = product_images.product_id
													WHERE products.product_id = '" . $product_id . "'
													GROUP BY i_client_business.BUSINESS_ID
													");
						$row = $query->row_array();
						...
						else
						{
							...
							//AGENT EMAIL
							if($row['agent_email'] != null){

								$mailsTt = array('email' => $row['agent_email']);
								array_push($mails, $mailsTt);
						...
						//IF BUSINESS LISTING AGENT CELL SEND SMS
						if($row['agent_cell'] != null){
							//SEND SMS
							$msgSMS = 'A question has been asked about listing ' . $this->shorten_string($product_title, 3) . '. Please answer it from your product dashboard. http://my.na/sell/my_trade/';
							...
							$user = $this->rest->get('send', array('number' => $row['agent_cell'], 'msg' => $msgSMS), 'json');

						}
						//IF BUSINESS LISTING CLIENT MOBILE VERIFIED SEND SMS
						if ($row['VERIFIED'] == 'Y')
						{
					...
					else
					{
						...
						//IF PRIVATE LISTING CLIENT MOBILE VERIFIED SEND SMS
						if ($row['VERIFIED'] == 'Y')
						{
					...
					if ($row['img_file'] != '')
					{
						...
						$img_str .= '<img src="' . base_url('/') . 'img/timbthumb.php?src=' . CDN_URL . 'assets/products/images/' . $row['img_file'] . '&w=190&h=130" />';
	...
	public function get_products($main_cat_id = 0, $sub_cat_id = 0, $sub_sub_cat_id = 0, $sub_sub_sub_cat_id = 0, $count = '', $offset = 0, $title = '', $amt = '', $advert = true, $pages = '')
	{
	...
	public function buy_now()
	{
		...
		if ($this->session->userdata('id') && $data['product_id'] = $this->input->post('product_id', true))
		{
			...
			if ($row['img_file'] != null)
			{
				...
				$images .= '<td style="width:100%;"><img src="' . base_url('/') . 'img/timbthumb.php?src=' . CDN_URL . 'assets/products/images/' . $row['img_file'] . '&w=580&h=320" style="width:100%; "></td>';


			}
			else
			{
				$images .= '<td style="width:100%;"><img src="' . base_url('/') . 'img/timbthumb.php?src=' . CDN_URL . 'img/product_blank.png&w=580&h=320" style="width:100%; "></td>';
...
//				$emailTO_buyer = array(array('email' => $rowB['CLIENT_EMAIL'] ));
		...
		if ($extras != null)
		{
			...
			foreach ($extras as $row => $value)
			{
				...
				else
				{
					...
					{
						$img = base64_encode(file_get_contents(BASE_URL . 'img/icons/trade/icn_chk.png'));
						$output .= '<tr>
											<td colspan="2">
					...
					{
						$img = base64_encode(file_get_contents(BASE_URL . 'img/icons/trade/icn_size.png'));
						$unit = substr(trim($value), strpos($value, ' '), strlen(trim($value)));
						if (is_numeric($unit))
						{
							$unit = 'm<sup>2</sup>';
					...
					elseif ($row == 'bedrooms' || $row == 'offices')
					{
						$img = base64_encode(file_get_contents(BASE_URL . 'img/icons/trade/icn_beds.png'));
						$output .= '<tr>
											<td colspan="2">
											<img src="data:image/png;base64,' . $img . '" style="width:20px;height:20px" width="20" height="20"/>
											' . ucwords($value) . ' 
					...
					elseif ($row == 'bathrooms')
					{
						$img = base64_encode(file_get_contents(BASE_URL . 'img/icons/trade/icn_baths.png'));
						$output .= '<tr>
											<td colspan="2">
											<img src="data:image/png;base64,' . $img . '" style="width:20px;height:20px" width="20" height="20"/>
											' . ucwords($value) . ' 
					...
					elseif ($row == 'parking' || $row == 'garages')
					{
						$img = base64_encode(file_get_contents(BASE_URL . 'img/icons/trade/icn_park.png'));
						$output .= '<tr>
											<td colspan="2">
											<img src="data:image/png;base64,' . $img . '" style="width:20px;height:20px" width="20" height="20"/>
					...
					elseif ($row == 'doors')
					{
						$img = base64_encode(file_get_contents(BASE_URL . 'img/icons/trade/icn_car_doors.png'));
						$output .= '<tr>
											<td colspan="2">
											<img src="data:image/png;base64,' . $img . '" style="width:20px;height:20px" width="20" height="20"/>
					...
					elseif ($row == 'body_style')
					{
						$img = base64_encode(file_get_contents(BASE_URL . 'img/icons/trade/icn_car_body.png'));
						$output .= '<tr>
											<td colspan="2">
											<img src="data:image/png;base64,' . $img . '" style="width:20px;height:20px" width="20" height="20"/>
											' . ucfirst($value) . '
					...
					elseif ($row == 'fuel_type')
					{
						$img = base64_encode(file_get_contents(BASE_URL . 'img/icons/trade/icn_car_petrol.png'));
						$output .= '<tr>
											<td colspan="2">
											<img src="data:image/png;base64,' . $img . '" style="width:20px;height:20px" width="20" height="20"/>
											' . ucfirst($value) . '
					...
					elseif ($row == 'transmission')
					{
						$img = base64_encode(file_get_contents(BASE_URL . 'img/icons/trade/icn_car_transmission.png'));
						$output .= '<tr>
											<td colspan="2">
											<img src="data:image/png;base64,' . $img . '" style="width:20px;height:20px" width="20" height="20"/>
											' . ucfirst($value) . '
					...
					elseif ($row == 'cylinders')
					{
						$img = base64_encode(file_get_contents(BASE_URL . 'img/icons/trade/icn_car_cylinders.png'));
						$output .= '<tr>
											<td colspan="2">
											<img src="data:image/png;base64,' . $img . '" style="width:20px;height:20px" width="20" height="20" />
											' . ucfirst(number_format((int) $value)) . ' ' . $row . '
					...
					elseif ($row == 'engine_size')
					{
						$img = base64_encode(file_get_contents(BASE_URL . 'img/icons/trade/icn_car_engine.png'));
						$output .= '<tr>
											<td colspan="2">
											<img src="data:image/png;base64,' . $img . '" style="width:20px;height:20px" width="20" height="20"/>
											' . ucfirst(number_format((int) $value)) . ' ' . $row . '
					...
					elseif ($row == 'year')
					{
						$img = base64_encode(file_get_contents(BASE_URL . 'img/icons/trade/icn_car_year.png'));
						$output .= '<tr>
											<td colspan="2">
											<img src="data:image/png;base64,' . $img . '" style="width:20px;height:20px" width="20" height="20"/>
					...
					elseif ($row == 'kilometres')
					{
						$img = base64_encode(file_get_contents(BASE_URL . 'img/icons/trade/icn_car_kilometers.png'));
						$output .= '<tr>
											<td colspan="2">
											<img src="data:image/png;base64,' . $img . '" style="width:20px;height:20px" width="20" height="20"/>
					...
					elseif ($row == 'color')
					{
						$img = base64_encode(file_get_contents(BASE_URL . 'img/icons/trade/icn_car_color.png'));
						$output .= '<tr>
											<td colspan="2">
											<img src="data:image/png;base64,' . $img . '" style="width:20px;height:20px" width="20" height="20"/>
											' . ucfirst($value) . ' 
					...
					elseif ($row == '4wd')
					{
						$img = base64_encode(file_get_contents(BASE_URL . 'img/icons/trade/icn_car_4x4.png'));
						$output .= '<tr>
											<td colspan="2">
											<img src="data:image/png;base64,' . $img . '" style="width:20px;height:20px" width="20" height="20" />
											' . ucfirst($value) . ' 
					...
					elseif ($row == 'owners')
					{
						$img = base64_encode(file_get_contents(BASE_URL . 'img/icons/trade/icn_car_owner.png'));
						$output .= '<tr>
											<td colspan="2">
											<img src="data:image/png;base64,' . $img . '" style="width:20px;height:20px" width="20" height="20"/>
											' . ucfirst($value) . ' 
					...
					elseif ($row == 'valuation')
					{
						$img = base64_encode(file_get_contents(BASE_URL . 'img/icons/trade/icn_cash.png'));
						$output .= '<tr>
											<td colspan="2">
											<img src="data:image/png;base64,' . $img . '" style="width:20px;height:20px" width="20" height="20"/>
	...
	function show_images_mobile($product_id, $size)
	{
		...
		else
		{
			...
			if ($size == 'big')
			{
				...
				$data .= '<div  class="white_box padding10"><img src="' . base_url('/') . 'img/product_blank_big.png" /></div>';
				$single = base_url('/') . 'img/product_blank_big.png';
			}
			else
			{

				$data .= '<img src="' . base_url('/') . 'img/product_blank.jpg" />';
				$single = base_url('/') . 'img/product_blank.jpg';
			}
	...
	function show_images2($product_id, $size)
	{
		...
		if ($images->result())
		{
			...
			foreach ($images->result() as $row)
			{
				...
				$med = base_url('/') . 'img/timbthumb.php?src=' . CDN_URL . 'assets/products/images/' . $row->img_file . '&w=700&h=520&' . $rand;

				$data .= '<div style="border:2px solid #fff" class="item ' . $active . ' white_box" data-slide-number="' . $x . '">
													<a href="' . CDN_URL . 'assets/products/images/' . $row->img_file . '?' . $rand . '" rel="prettyPhoto[gal]" ><img class="lazy image_' . $x . '" itemprop="image" data-original="' . $med . '" src="' . base_url('/') . 'img/deal_place_load.gif" /></a>
												  </div>';
				$x++;
			}
			$data .= '</div>
	...
	function show_images_mosaic($images, $size)
	{
			...
			foreach ($imgA as $row)
			{
				...
				if ($x == 0)
				{
					...
					$data['str'] .= '<li class="span6  mosaic">
								  <a href="#"  style="background:#fff;">
									<img alt="" style="min-height:280px;min-width:380px" class="white_box padding2" src="' . base_url('/') . 'img/timbthumb.php?src=' . CDN_URL . 'assets/products/images/' . $row . '&w=420&h=350' . $rand . '" >
								  </a>
								</li>';
					$data['file'] = base_url('/') . 'img/timbthumb.php?src=' . CDN_URL . 'assets/products/images/' . $row . '&w=420&h=330';
				...
				elseif ($x <= 4)
				{
					...
					$data['str'] .= '<li class="span3 hidden-phone">
								  <a href="#"  style="background:#fff;">
									<img alt="" class="white_box padding2" src="' . base_url('/') . 'img/timbthumb.php?src=' . CDN_URL . 'assets/products/images/' . $row . '&w=190&h=150' . $rand . '">
								  </a>
								</li>';
		...
		else
		{
			...
			$data['str'] = '';
			$data['file'] = base_url('/') . 'img/timbthumb.php?src=' . base_url('/') . 'img/product_blank.jpg';
		...
		if ($bid->result())
		{
			...
			foreach ($bid->result() as $row)
			{
				...
				<section class="results-item">
					...
					<div>
					  <a title="' . $client['name'] . ' placed a bid of ' . $bid . '" rel="tooltip" class="btn btn-success pull-right text-light"><i class="fa fa-plus"></i> ' . $bid . '</a> 
	...
	function add_product_images()
	{
		...
		if (isset($_FILES['files']['name']))
		{
			...
			for ($i = 0; $i < $this->total_count_of_files; $i++)
			{
				...
				$config['upload_path'] = BASE_URL . 'assets/products/images/';
				...
				if ($this->upload->do_upload())
				{
					...
					if($this->input->get('classifieds')){
						$data = array(
							...
							'img_file'   => $file,
							'classified_id' => $product_id,
							
	
						);
						//insert into database
						$this->db->insert('classifieds_images', $data);
					...
					//UPLOAD S3
					if(file_exists(BASE_URL.'/assets/products/images/' . $file)){
						
						$data['out'] = $this->cron_model->upload_s3('assets/products/images/' . $file);
					}else{
...
//+++++++++++++++++++++++++++
		...
		if (isset($_FILES['files']['name']))
		{
			...
			for ($i = 0; $i < $this->total_count_of_files; $i++)
			{
				...
				if ($this->upload->do_upload())
				{
					...
					$tt = system('wget '.site_url('/') .'cron_jobs/upload_s3/?file=/assets/products/images/' . $file);
					
					//$tt = file_get_contents(site_url.'cron_jobs/upload_s3/?file=/assets/products/images/' . $file);
					if($tt){
						
						$data['out'] = $tt;
					}else{
						
						$data['out'] = 'Not shifted'.$tt;
	...
	function downsize_image($file)
	{
		...
		$config = array(
			'source_image'   => (BASE_URL . 'assets/products/images/' . $file),
			'width'          => '900',
			'master_dim'     => 'width',
			'maintain_ratio' => true
		);
		...
		/*$config = array( 
		   ...
		   'height' => '200',
		   'quality' => 10,
		   'new_image' => (BASE_URL .'assets/products/images/thumb_'. $file),
	...
	function watermark_image($file)
	{
		...
		$config['source_image'] = BASE_URL . 'assets/products/images/' . $file;
		...
		if ($main->result())
		{
			...
			foreach ($main->result() as $row)
			{
				...
				if ($row->IS_ESTATE_AGENT == 'Y')
				{
					...
					if (trim($row->BUSINESS_LOGO_IMAGE_NAME) != '')
					{
						$b_logo = '<img title="Product is listed by ' . $row->BUSINESS_NAME . '" rel="tooltip" style="margin-top:-70px;z-index:1;position:relative" src="' . base_url('/') . 'img/timbthumb.php?w=50&h=50&src=' . base_url('/') . 'assets/business/photos/' . $row->BUSINESS_LOGO_IMAGE_NAME . '" alt="' . $row->BUSINESS_NAME . '" class="img-polaroid pull-right" />';
					}
					else
					{
						$b_logo = '<img title="Product is listed by ' . $row->BUSINESS_NAME . '" rel="tooltip" style="margin-top:-70px;z-index:1;position:relative" src="' . base_url('/') . 'img/timbthumb.php?w=50&h=50&src=' . base_url('/') . 'img/bus_blank.jpg" alt="' . $row->BUSINESS_NAME . '" class="img-polaroid pull-right" />';
					}
