<?php
class Scratch_model_external extends CI_Model{
	public function __construct()
    {
        // Constructor's functionality here, if you have any.
    }
	
 	function scratch_model_external(){
  		//parent::Model();
		self::__construct();				
 	}
	//+++++++++++++++++++++++++++
	//SCRATCH & WIN MODEL
	//FOR EXTERNAL PROMOTIONS
	// 
	// Roland Ihms
	//++++++++++++++++++++++++++
...
//				//GET WINNING IMAGE
//				$valid['img_file'] = 'price_lost_'.rand(1,6).'.jpg';
//
//				//UPDATE SCRATCH PLAYS TABLE - DEDUCT 10 POINTS
//				$this->log_new_play($prize['ID'],$prize['PROMOTION_ID'], $valid['img_file'], $coupon);	
//		
...
//					WHERE DATE_FORMAT(CREATED_AT,'%Y-%m-%d') = '".$date."' ORDER BY CREATED_AT DESC");
    ...
    public function nextPrize($promo_id) 
    {
		...
		}else{//end if prizes available
			
			$total = 0;
			$prize['ID'] = 0;
			$prize['NAME'] = 'EMPTY PRIZE';
			$prize['PROMOTION_ID'] =
			$prize['IMAGE_URL'] = 'price_win_buffet.jpg';
			$prize['QUANTITY'] = 0;
			$prize['BUSINESS_ID'] = 1290;
			$prize['total'] = 0;	
			return $prize;
    ...
    public function sendWinNotifications($prize_id, $promo_id, $user_id, $coupon, $prize_img)
    {
		...
		$body = '<img src="'.$prize_img.'" alt="Download Images to view Prize" style="float:right;"/>
				...
				$coupon . '</font><br /><br /> You can contact the business by replying to this email or by contacting them at <em>'.$business['BUSINESS_EMAIL'].'</em> <h4>Special instructions</h4>'
		...
		if($query->result()){
			...
			<div class="prize_cover" style="width:250px; height:338px;"></div>
				...
				foreach($query->result() as $row){
					...
					echo '<div class="item '.$active.'" style="text-align:center"><img src="'.base_url('/').'scratch_card/images/prizes/raw/'.str_replace('_win','',$row->IMAGE_URL).'" /></div>';
					$x ++;
				}
