...
class timthumb {
	...
	public function __construct(){
		...
		$src = $this->param('src');
		//Patch for app
		$firstL = substr($src,0,1);

		if($firstL == '/'){
			$src = '/d3rp5jatom3eyn.cloudfront.net'.$src;
		}

		$this->src = str_replace('/wwww.my.na/NEW/','/d3rp5jatom3eyn.cloudfront.net/',$src);
		$this->src = str_replace('/wwww.my.na/','/d3rp5jatom3eyn.cloudfront.net/',$src);
		...
		//$this->url['host'] = 'd3rp5jatom3eyn.cloudfront.net'; 
		//PATCH REDIRECT
		if(isset($this->url['host']) && $this->url['host'] == 'www.my.na'){
			
			$this->url['host'] = 'd3rp5jatom3eyn.cloudfront.net'; 
			$this->src = 'https://d3rp5jatom3eyn.cloudfront.net'.$this->url['path']; 
		}
		
		//CLEAN PATH REMOVE NEW
		if(substr($this->url['path'],0,4) == '/NEW'){

			$this->url['path'] = substr($this->url['path'],4, strlen($this->url['path']));
			$this->src = 'https://d3rp5jatom3eyn.cloudfront.net'.$this->url['path']; 
		}
		...
		if(!isset($this->url['host'])){
			...
			$this->url['host'] = 'd3rp5jatom3eyn.cloudfront.net';
			$this->url['path'] = str_replace( 'd3rp5jatom3eyn.cloudfront.net/', '', $this->url['path']);
			$this->src = 'https://d3rp5jatom3eyn.cloudfront.net'.$this->url['path'];

		}
		//Redirect Internal Image to S3
		if(substr($this->url['path'],0,7) == '/assets'){

			//$this->url['path'] = substr($this->url['path'],7, strlen($this->url['path']));
			$this->src = 'https://d3rp5jatom3eyn.cloudfront.net'.$this->url['path']; 
		}

		//var_dump($this->url);
		//echo substr($this->url['path'],0,4);
